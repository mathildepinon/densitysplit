...
class DensitySplitFit:
    ...
    def _chi_square(self, reduced=True):
        ...
        if self.fitted:
            model_params = copy.deepcopy(self.fixed_params)
            model = self.xi_model.split_model(s=s, params=model_params.update(self.popt_dict))
    ...
    def draw_contour(self, params, free_params_init=None, fig=None, ax=None, size=100, cl=None, s_lower_limit=None):
        ...
        m = self.minuit
        # m.limits['b_DS1'] = (0.5, 5)
        # m.limits['b_DS2'] = (0.5, 5)
        # m.limits['b_DS3'] = (0.5, 5)
        # m.limits['alpha_par'] = (0.5, 1.5)
        # m.limits['alpha_perp'] = (0.5, 1.5)
        # m.limits['sigma_par'] = (3, 15)
        # m.limits['sigma_perp'] = (3, 15)
        # for split in range(self.nsplits):
        #     for ill in range(self.nells):
        #         for k in range(3):
        #             m.limits['bb{}_{}_DS{}'.format(k, ill, split+1)] = (-100, 100)
        ...
        x = [x[0] for x in self.mncontour]
        x.append(x[0])
        y = [y[1] for y in self.mncontour]
        y.append(y[0])

        min_location = [self.popt_dict[params[0]], self.popt_dict[params[1]]]
        hesse_errors = [self.pstd_dict[params[0]], self.pstd_dict[params[1]]]
